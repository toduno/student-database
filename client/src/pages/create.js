...
export default function Create() {
    ...
    //stringify (as json when sending to the server) unless the property is an object
    //so it'll be stringified only
    ...
    async function onSubmit(e) {
        ...
        await fetch(`http://localhost:7000/record/add`, {
            method: 'POST',
            body: formData
            //N.B: Ince we've added the encType attrib to the <form>, the server will know that an image is included
            //in the form so here, no need to speify the content-header for the POST request as encType again or leave it
        ...
        <div className='flex flex-col md:flex-row'>
            ...
            <div className='flex flex-col items-center justify-center px-2 md:pl-7 py-7 md:pt-4 md:pb-8 md:w-[50%]'>
                ...
                <form onSubmit={onSubmit} encType='multipart/form-data' className='w-full flex flex-col gap-y-4 md:gap-y-5'>
                    ...
                    <div>
                        <input type='submit' value="Create record" className='md:mt-2 mt-1 font-semibold bg-red-700 hover:bg-red-600 px-4 md:px-8 py-2 text-white' />
                    </div>
                </form>
            </div>
        </div>
